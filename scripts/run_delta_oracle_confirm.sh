#!/usr/bin/env bash
set -euo pipefail

# Confirm delta-oracle stability for a fixed proposal config.
# Default config: w=opacity, rc=5.0, rf=0.25, kc=200, kf=400

usage() {
  cat <<'EOF'
Usage:
  bash scripts/run_delta_oracle_confirm.sh \
    --source_path /path/to/dataset \
    --model_path ./experiment/scene \
    [--images images_4] \
    [--iteration 3000] \
    [--views "0,10,20,30,40"] \
    [--alpha0 1e-2 --scale_factor 1.0] \
    [--k_list "50,100,200,400"] [--num_trials 50] \
    [--weight_num_views 1 --weight_view_stride 1 --weight_view_indices ""]

Outputs:
  <model_path>/oracle_verify/delta_oracle_confirm/<tag>/view_<v>/delta_oracle_summary.csv
  <model_path>/oracle_verify/delta_oracle_confirm/<tag>/confirm_summary.csv
EOF
}

SOURCE_PATH=""
MODEL_PATH=""
IMAGES=""
ITERATION="3000"
VIEWS="0,10,20,30,40"
ALPHA0="1e-2"
SCALE_FACTOR="1.0"
K_LIST="50,100,200,400"
NUM_TRIALS="50"
WEIGHT_NUM_VIEWS=""
WEIGHT_VIEW_STRIDE=""
WEIGHT_VIEW_INDICES=""

W_MODE="opacity"
RC="5.0"
RF="0.25"
KC="200"
KF="400"

while [[ $# -gt 0 ]]; do
  case "$1" in
    --source_path) SOURCE_PATH="$2"; shift 2;;
    --model_path) MODEL_PATH="$2"; shift 2;;
    --images) IMAGES="$2"; shift 2;;
    --iteration) ITERATION="$2"; shift 2;;
    --views) VIEWS="$2"; shift 2;;
    --alpha0) ALPHA0="$2"; shift 2;;
    --scale_factor) SCALE_FACTOR="$2"; shift 2;;
    --k_list) K_LIST="$2"; shift 2;;
    --num_trials) NUM_TRIALS="$2"; shift 2;;
    --weight_num_views) WEIGHT_NUM_VIEWS="$2"; shift 2;;
    --weight_view_stride) WEIGHT_VIEW_STRIDE="$2"; shift 2;;
    --weight_view_indices) WEIGHT_VIEW_INDICES="$2"; shift 2;;
    -h|--help) usage; exit 0;;
    *) echo "Unknown arg: $1"; usage; exit 1;;
  esac
done

if [[ -z "${SOURCE_PATH}" || -z "${MODEL_PATH}" ]]; then
  echo "Error: --source_path and --model_path are required."
  usage
  exit 1
fi

TAG="w_${W_MODE}_rc_${RC}_rf_${RF}_kc_${KC}_kf_${KF}"
OUT_BASE="${MODEL_PATH}/oracle_verify/delta_oracle_confirm/${TAG}"
mkdir -p "${OUT_BASE}"

COMMON_ARGS=(--source_path "${SOURCE_PATH}" --model_path "${MODEL_PATH}" \
  --split train --iteration "${ITERATION}" --alpha0 "${ALPHA0}" --scale_factor "${SCALE_FACTOR}" \
  --k_list "${K_LIST}" --num_trials "${NUM_TRIALS}" \
  --proposal_mode gaussian_perturb --w_mode "${W_MODE}" \
  --rc "${RC}" --rf "${RF}" --kc "${KC}" --kf "${KF}")

if [[ -n "${IMAGES}" ]]; then
  COMMON_ARGS+=(--images "${IMAGES}")
fi
if [[ -n "${WEIGHT_NUM_VIEWS}" ]]; then
  COMMON_ARGS+=(--weight_num_views "${WEIGHT_NUM_VIEWS}")
fi
if [[ -n "${WEIGHT_VIEW_STRIDE}" ]]; then
  COMMON_ARGS+=(--weight_view_stride "${WEIGHT_VIEW_STRIDE}")
fi
if [[ -n "${WEIGHT_VIEW_INDICES}" ]]; then
  COMMON_ARGS+=(--weight_view_indices "${WEIGHT_VIEW_INDICES}")
fi

IFS=',' read -ra VIEW_LIST <<< "${VIEWS}"
for v in "${VIEW_LIST[@]}"; do
  v="$(echo "$v" | xargs)"
  out_dir="${OUT_BASE}/view_${v}"
  echo "=== iter=${ITERATION}, view=${v}, ${TAG} ==="
  python scripts/estimate_delta_oracle.py \
    "${COMMON_ARGS[@]}" \
    --view_index "${v}" \
    --out_dir "${out_dir}"
done

python - "${OUT_BASE}" "${ITERATION}" "${TAG}" <<'PY'
import csv, math, os, sys
base_dir, iteration, tag = sys.argv[1:]
rows = []
for root, _, files in os.walk(base_dir):
    if "delta_oracle_summary.csv" not in files:
        continue
    path = os.path.join(root, "delta_oracle_summary.csv")
    per = {}
    with open(path, "r", encoding="utf-8") as f:
        lines = [ln.strip() for ln in f if ln.strip()]
    i = 0
    while i < len(lines):
        if lines[i].startswith("#") and "per_run" in lines[i]:
            if i + 2 < len(lines):
                header = next(csv.reader([lines[i+1]]))
                row = next(csv.reader([lines[i+2]]))
                per = dict(zip(header, row))
                break
        i += 1
    if per:
        rows.append(per)

def fnum(x):
    try:
        return float(x)
    except Exception:
        return float("nan")

out_csv = os.path.join(base_dir, "confirm_summary.csv")
with open(out_csv, "w", encoding="utf-8", newline="") as f:
    w = csv.writer(f)
    w.writerow(["iteration","view_index","g_min","delta_kmax","delta_norm","visible_ratio"])
    for r in rows:
        gmin = fnum(r.get("g_min","nan"))
        # parse Kmax delta
        kmax = None
        delta_kmax = float("nan")
        # parse by_K section
        byk = []
        path = os.path.join(os.path.dirname(r.get("__path","")), "delta_oracle_summary.csv")
        # best-effort: re-open in this root
        for root, _, files in os.walk(base_dir):
            if "delta_oracle_summary.csv" in files and root.endswith(f"view_{r.get('view_index','')}"):
                path = os.path.join(root, "delta_oracle_summary.csv")
                break
        if os.path.isfile(path):
            with open(path, "r", encoding="utf-8") as f2:
                lines = [ln.strip() for ln in f2 if ln.strip()]
            i = 0
            while i < len(lines):
                if lines[i].startswith("#") and "by_K" in lines[i]:
                    i += 1
                    if i < len(lines) and lines[i].startswith("K,"):
                        i += 1
                    while i < len(lines) and not lines[i].startswith("#"):
                        row = next(csv.reader([lines[i]]))
                        if len(row) >= 2:
                            byk.append((int(float(row[0])), fnum(row[1])))
                        i += 1
                    break
                i += 1
        if byk:
            byk.sort(key=lambda x: x[0])
            kmax, delta_kmax = byk[-1]
        delta_norm = delta_kmax / (abs(gmin) + 1e-12) if not math.isnan(delta_kmax) and abs(gmin) > 0 else float("nan")
        w.writerow([iteration, r.get("view_index",""), gmin, delta_kmax, delta_norm, r.get("visible_ratio","")])
print("Wrote", out_csv)
PY

echo "Confirm summary: ${OUT_BASE}/confirm_summary.csv"
